...
| 2026-06-24 | Workflow run Source panel exceeds the 360–420px details-rail width (§6) and is user-resizable | The panel renders unwrapped workflow YAML, which the 420px rail truncates badly. It defaults to 720px and drags between 420px and `min(1280px, 85vw)` via a left-edge separator (pointer + ArrowLeft/ArrowRight), so the run content beside it never collapses. The details-rail range still governs metadata rails; this exception is scoped to code/source surfaces. |
