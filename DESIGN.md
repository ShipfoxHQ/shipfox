...
| 2026-06-28 | Per-step "View source" is an icon-only, hover/focus-revealed row action on the step row's far right (Remix `codeSSlashLine`, `transparentMuted`); the in-panel highlight scrolls the step's lines to center | Follows §10's "inline row actions revealed on hover" so the affordance stays quiet until needed, while remaining keyboard-reachable (revealed on `focus-visible`, never `display:none`) and forced visible while its panel is open. The button is a sibling of the Accordion disclosure (not nested in its trigger/heading) to keep valid HTML. One control per step (the current-attempt "anchor" row, computed in the pure step-list model) keeps the shared panel's `aria-expanded` mutually exclusive across the summary and step buttons. Scroll-to-highlight is an opt-in `scrollHighlightedIntoView` on the react-ui `CodeBlockContent` primitive because only it observes when the async Shiki markup (the `.highlighted-line` target) has rendered; it reads `prefers-reduced-motion` at scroll time and is guarded so layout-less environments never throw. |
