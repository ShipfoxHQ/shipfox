...
| 2026-06-29 | Job graph nodes show a single bare duration (mono, muted, tabular): a live queue/run timer or a final span, with no verb | A job has two honest clocks — wait (`now − queued_at`) before it runs and execution (`(finished_at ?? now) − started_at`) once it does — derived client-side from the existing job timestamps; the backend is unchanged. The node's status glyph already names the state, so the duration carries no verb (the screen-reader label does: "queued 6m 00s" / "ran 2m 14s"), keeping one status affordance per dense node (§10). When the relevant anchor is null (skipped, cancelled-before-start, or pre-projection lag) the node shows nothing rather than a placeholder (honest UI). A shared ticker (visibility-paused) advances the live values every second, slowing to a calm 10s cadence under reduced motion so the timer keeps advancing without per-second motion. |
