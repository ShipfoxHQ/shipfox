...
{/* Source: images/diagrams/run-execution-flow.mmd — regenerate with:
    npx -y @mermaid-js/mermaid-cli -b white -i images/diagrams/run-execution-flow.mmd -o images/diagrams/run-execution-flow.svg */}
<Frame caption="How a run executes — click to enlarge.">
  <img src="/images/diagrams/run-execution-flow.svg" alt="Run execution flow: event sources match a trigger in the Shipfox control plane, a run is created with event data resolved into prompts, jobs are scheduled as a needs DAG, a runner on your compute polls outbound and claims the job, then executes run and agent steps with a gate looping back on failure while logs and agent sessions stream live to the dashboard" />
</Frame>
