...
- [ ] **Sticky `+ Create project` footer in project-switcher** — Port the three-step transform from the workspace-switcher fix on `noe-charmet/static-create-workspace-button` to `libs/client/projects/src/components/project-switcher.tsx`: drop the redundant `ScrollArea` wrapper, hoist `CommandSeparator` + the create `CommandGroup` out of `CommandList`, add `forceMount` to the create `CommandItem`. Identical UX bug — once a workspace has enough projects to overflow, the "Create project" item scrolls away with the list. ~6 line diff. Pairs visually with workspace-switcher in the top nav and should land soon to keep the two consistent. Depends on the workspace-switcher fix landing first.

- [ ] **VRT snapshot for many-workspaces overflow** — The new `auth/workspace-switcher-open` and `auth/workspace-switcher-empty` Argos snapshots (added on `noe-charmet/static-create-workspace-button`) prove the sticky-footer + `forceMount` behavior, but the seeded E2E user only has one workspace — the actual scroll/overflow case isn't visually covered. Add an Argos snapshot of `WorkspaceSwitcher` with enough workspaces to overflow `max-h-300`, proving the footer stays pinned during scroll. Blocked on a bulk-workspace seed helper in `@shipfox/e2e-helper-auth` (today the helper creates one workspace per user via onboarding); pair with any future bulk-seed work for other UI tests.
