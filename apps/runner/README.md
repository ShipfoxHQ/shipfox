...
| `SHIPFOX_RUNNER_ENABLE_RENEWABLE_INFERENCE` | `false` | Enables renewable managed inference credentials. Verified managed images set this to `true` after the helper and production-closure checks pass. Keep `false` on self-hosted or unverified images. |
