...
| `runner` | `string \| string[]` | Default runner label or list of labels for all jobs in this workflow. Individual jobs can override this value with their own `runner` field. See [Runners and execution environments](/understand/runners-and-execution-environments) for how labels route jobs. |
...
| `config` | `object` | Source-specific trigger settings. Cron uses `schedule` and optional `timezone`; see [Cron triggers](/reference/trigger-sources#cron). Other shipped sources do not accept this field. |
...
| `success` | `string` | A CEL expression that decides whether the job succeeded once all its executions settle, with `executions` in scope (each element carries `index`, `name`, `status`, `events`, `outputs`, and timing fields). Defaults to succeeding when no execution failed, including an empty listener. See [Expressions](/reference/expressions#job-success-success). |
...
The workflow receives the selected operations, not the integration connection credential.
...
- **Values may interpolate.** `${{ }}` expressions resolve when their referenced
  context becomes available; see [Expressions](/reference/expressions).
...
<Callout type="warn">
  ...
  - **Write-capable integration tools require `allow_write: true`.** Unknown integration connections, tools, and methods are rejected during sync.
  ...
  review:
    ...
    steps:
      - key: review
        ...
        thinking: high
        prompt: Review the repository and suggest improvements.
```
