...
<ParamField body="listening" type="object">
  Turns the job into a [listening job](/concepts/listening-jobs) that reacts to events inside the run (🚧 coming soon — the field parses and validates, but listening jobs do not execute yet).
</ParamField>
