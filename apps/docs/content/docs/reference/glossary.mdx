...
A condition block attached to a step that controls success evaluation and failure recovery. The `success` field accepts a CEL expression over `step.exit_code`, `step.status`, and `step.outputs`. The `on_failure.restart_from` field names an earlier keyed step in the same job to restart from when the gate fails, and `on_failure.feedback` is an optional message the restarted step can read. The optional `on_failure.max_attempts` field bounds executions of the gate, including the first execution. Reaching that bound reports `restart_exhausted`; an execution timeout remains `timed_out`. A gate must contain at least one of `success` or `on_failure`. See [Feedback loops](/understand/feedback-loops), [Limits](/reference/limits), and [Gate fields](/reference/workflow-schema#gate-fields).
