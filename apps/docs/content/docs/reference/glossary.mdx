...
A condition block attached to a step that controls success evaluation and failure recovery. The `success` field accepts a CEL expression over `step.exit_code`, `step.status`, and `step.outputs`. The `on_failure.restart_from` field names an earlier keyed step in the same job to restart from when the gate fails. The optional `on_failure.max_attempts` field bounds gating-step executions, including the first execution. The optional `on_failure.feedback` field supplies evidence the restarted step can read. A gate must contain at least one of `success` or `on_failure`. A gate that still fails at its limit reports `restart_exhausted`; a job that reaches its execution timeout reports `timed_out`. See [Feedback loops](/understand/feedback-loops) and [Gate fields](/reference/workflow-schema#gate-fields).
