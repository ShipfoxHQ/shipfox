...
predicates. See [Manage secrets and
variables](/how-to/set-up-work/secrets-and-variables) and [Context
availability](/reference/contexts#context-availability).
...
Controls reasoning for agent steps. Set the `thinking` field on an agent step. Higher levels spend more tokens on internal reasoning before a response. They can improve quality for complex tasks. Accepted values depend on the harness. Omitting the field uses configured defaults or `xhigh`. Set `default` to use the provider default without workspace or deployment overrides. See [Agent step fields](/reference/workflow-schema#agent-step-fields).
...
for non-sensitive config; use secrets for credentials. See [Manage secrets and
variables](/how-to/set-up-work/secrets-and-variables).
