...
| --- | --- | --- | --- |
| Gate attempts per gating step | 5 (including the first execution) | Per gate, in YAML | `gate.on_failure.max_attempts` accepts integers from 1 through 1,000. Omit it for five on a new run. Active and rerun executions with no persisted limit retain three. There is no `unlimited` value. When the cap is exhausted, the step fails with `restart_exhausted`. See [Feedback loops](/understand/feedback-loops#loop-bounds). |
