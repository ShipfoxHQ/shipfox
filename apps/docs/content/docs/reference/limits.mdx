...
sidebarTitle: "Limits"
description: "Look up Shipfox gate attempt caps, job timeouts, log budgets and retention, output grouping caps, environment-map limits, workspace secret and variable caps, and agent session caps."
---
...
| Run timeout | 30 days | No (not authorable) | Outer bound on a whole run, including its listening jobs. Time spent waiting for a concurrency group does not count toward this timeout. A listening job's `timeout` is capped by it. |
| Manual triggers per workflow | 1 | No | See [Trigger fields](/reference/workflow-schema#trigger-fields). |
| Concurrency with listening jobs | Not supported | No | A workflow cannot use `concurrency` when it has a listening job. See [Concurrency fields](/reference/workflow-schema#concurrency-fields). |
...
| Limit | Default | Configurable | Details |
| --- | --- | --- | --- |
| Secrets and variables per workspace | 10,000 | Self-host: `SECRETS_MAX_PER_WORKSPACE` | One cap counts secrets and variables together. See [Manage secrets and variables](/how-to/set-up-work/secrets-and-variables). |

## Workflow chaining

| Limit | Default | Configurable | Details |
| --- | --- | --- | --- |
| Child-run depth | 5 | No | A run at depth 5 cannot start another workflow. The `start_workflow_run` tool returns `run-depth-exceeded`. |
| Descendant runs per root run | 100 | No | Shipfox rejects another child after the run tree reaches 100 descendants. The `start_workflow_run` tool returns `run-tree-limit-exceeded`. |
