...
| --- | --- | --- | --- |
| Gate attempts per gating step | 5 (including the first execution) | 1 to 1,000 per gate, in YAML | `gate.on_failure.max_attempts` counts the first execution. Omit it for five attempts on a new run. Active and rerun executions with no persisted limit retain the legacy limit of three. Exhaustion reports `restart_exhausted`. There is no `unlimited` value. See [Feedback loops](/understand/feedback-loops#loop-bounds). |
| Job `execution_timeout` | 6 hours | Per job, in YAML | Wall-clock bound on one job execution, including gate retries. Authored values range from 1 second through 24 hours. A timeout reports `timed_out`, not `restart_exhausted`. See [Job fields](/reference/workflow-schema#job-fields). |
...
| Limit | Default | Configurable | Details |
| --- | --- | --- | --- |
| Per-job log budget | 32 MiB + 1 MiB per minute | Self-host: `LOG_BUDGET_BASE_BYTES`, `LOG_BUDGET_RATE_BYTES_PER_MINUTE` | The budget limits retained output, not execution. After the budget is reached, later attempts can have incomplete or no logs. A `capped` record marks the boundary. See [Inspect logs](/how-to/run-and-troubleshoot/inspect-logs). |
