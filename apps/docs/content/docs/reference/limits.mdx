...
| --- | --- | --- | --- |
| Gate attempts per gating step | 5 (including the first execution) | Per gate, in YAML | Authors can set `gate.on_failure.max_attempts` from 1 through 1,000. The value includes the first execution. New runs use five when the field is omitted. Active and rerun executions with no persisted limit retain the legacy limit of three. There is no `unlimited` value. When the cap is exhausted and the gate still fails, the step is marked failed with `restart_exhausted` and the job stops. See [Feedback loops](/understand/feedback-loops#loop-bounds). |
...
| Limit | Default | Configurable | Details |
| --- | --- | --- | --- |
| Per-job log budget | 32 MiB + 1 MiB per minute | Self-host: `LOG_BUDGET_BASE_BYTES`, `LOG_BUDGET_RATE_BYTES_PER_MINUTE` | The budget limits retained output, not execution. Past the budget, further output is dropped and a `capped` record marks the boundary. Gate retries continue, but later attempts can have incomplete or no logs. See [Inspect logs](/how-to/run-and-troubleshoot/inspect-logs). |
