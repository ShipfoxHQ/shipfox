...
The `key` field identifies the step — in the dashboard and as the target of a `restart_from` gate. Use it on any step you may want to reference.
...
The `thinking` field controls the model's reasoning depth before it produces output. The accepted values depend on the step's harness, and the default is `xhigh` — see the [agent step fields reference](/reference/workflow-schema#agent-step-fields).

Higher thinking levels increase reasoning depth and token usage. Use `low` for straightforward summarization or simple generation tasks. Use `high` or above for complex code review, multi-file debugging, or any task where accuracy matters more than speed.
...
<Callout type="warn">
  ...
  - `run` and any of `prompt`, `model`, `harness`, `thinking`, or `provider` cannot appear on the same step.
