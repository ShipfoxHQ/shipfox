...
**GitHub App**. Once connected, pushes and other repository events that the App
subscribes to can trigger workflow runs.
...
The branch ref is exposed to filters as `event.ref`, using the **full Git ref** (for
...
<Callout type="info">
  The filter runs when the event arrives: a push that does not match starts no
  run. An expression error fails closed (the trigger does not fire), so test a
  new filter with a push on a scratch branch first.
</Callout>

## Related pages

<Cards>
  <Card title="Checks on push" href="/how-to/patterns/checks-on-push">
