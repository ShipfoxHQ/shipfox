...
can trigger on. Use it for tools Shipfox does not integrate with directly, such as CI
...
<Callout type="info">
  ...
  `event.body`. It is evaluated when the request arrives. A request that does
  not match starts no run.
</Callout>
