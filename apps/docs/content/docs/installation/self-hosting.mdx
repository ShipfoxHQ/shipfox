...
A runner is a lightweight process you start on any machine, such as a VM, a bare-metal
...
3. Start the runner on your target machine. The runner ships as a Docker image and is configured entirely through environment variables. Set the API URL, registration token, and labels that this runner advertises:
...
<Callout type="info">
  ...
  [runner provisioner](/operations/runner-provisioners). It starts ephemeral,
