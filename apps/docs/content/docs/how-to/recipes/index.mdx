---
title: "Workflow Recipes"
description: "Start with a complete workflow for a common task, then adapt it to your repository."
---

Each recipe gives you a full workflow for a common task. It assumes you know
the basic workflow parts from [Understand](/understand). Replace the values
noted in the recipe, then run it and change the commands or prompts to fit your
repository.

Use the [integrations catalog](/integrations) to connect a provider before you
use a recipe that needs one.

<Cards>
  <Card title="Run checks on every push" href="/how-to/recipes/checks-on-push">
    Run tests before an agent reviews each GitHub push.
  </Card>
  <Card title="Triage new Sentry issues" href="/how-to/recipes/triage-sentry-issues">
    Start an agent investigation when Sentry reports an issue.
  </Card>
  <Card title="Run parallel checks, then review" href="/how-to/recipes/parallel-ci-agent-review">
    Run lint and tests together, then start an agent review after both pass.
  </Card>
  <Card title="Fix Sentry issues with a pull request" href="/how-to/recipes/event-to-verified-pull-request">
    Let an agent fix an issue, pass the tests, push a branch, and open a pull request.
  </Card>
  <Card title="Retry agent work until checks pass" href="/how-to/recipes/build-feedback-loop">
    Check agent work and retry it with evidence.
  </Card>
  <Card title="Batch pull request feedback" href="/how-to/recipes/bound-listening-job">
    Group bursts of review comments and guarantee that the listener stops.
  </Card>
</Cards>

Use the [Workflow Schema](/reference/workflow-schema) to look up fields. Read
[Understand](/understand) to learn why each part works this way.
