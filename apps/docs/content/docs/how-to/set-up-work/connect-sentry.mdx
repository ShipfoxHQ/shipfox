...
Use the [Sentry reference](/integrations/sentry) for event names and event
fields instead of copying Sentry's event catalog. The [Sentry triage
recipe](/how-to/recipes/triage-sentry-issues) turns a delivered issue into an
agent run.
