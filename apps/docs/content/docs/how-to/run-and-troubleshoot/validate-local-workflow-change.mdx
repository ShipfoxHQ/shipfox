...
Use this guide to validate unpushed workflow YAML against a retained event
before you start a run. The dry-run call checks the definition and trigger
without executing workflow steps.
...
- A Shipfox project with its repository and integration connections configured.
- A workflow YAML file in your working tree.
- A matching event received within the last 30 days.

Shipfox retains trigger events for 30 days by default. Use a newer event when
the event you need no longer appears.

<Callout title="Treat event payloads as untrusted data" type="warn">
...
## Choose an event as test input

Ask the coding agent to follow these steps:
...
When the check passes, [test the local workflow change](/how-to/run-and-troubleshoot/test-local-workflow-change)
against the event to inspect its real behavior.
