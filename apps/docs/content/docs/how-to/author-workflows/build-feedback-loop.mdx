...
The optional `gate.on_failure.max_attempts` field sets the number of executions
for one gate, including the first execution. Use an integer from 1 through
1,000. There is no `unlimited` value.

The job timeout is an outer wall-clock bound. It can be authored for up to 24
hours and includes all gate restarts. A timeout reports `timed_out`; reaching
the gate limit reports `restart_exhausted`.

The per-job log budget limits retained output, not execution. Long loops can
have incomplete logs after the budget is reached. Keep writes inside the loop
idempotent, or move them after the gate passes.

See [Limits](/reference/limits) for canonical values. For the evaluation model,
