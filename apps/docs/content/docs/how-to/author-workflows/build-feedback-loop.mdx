...
`max_attempts` counts the gating step's executions, including the first one.
Use `1` to disable automatic restarts after the initial failure. Values from `1`
through `1,000` are supported. Omit the field to use five attempts on a new run.
There is no `unlimited` value.

The `execution_timeout` is a separate wall-clock bound. It can be set to at
most 24 hours, including gate restarts. A timeout reports `timed_out`; reaching
the attempt limit reports `restart_exhausted`. The retained log budget limits
stored output, not execution, so long loops can have incomplete later logs.
Keep external writes after the gate, or make them idempotent with a stable key.
