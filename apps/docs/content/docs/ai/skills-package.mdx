...
This installs the skill via [skills.sh](https://skills.sh) and makes it available to Claude Code, Cursor, Codex, and 18+ other agent frameworks automatically. No manual configuration is required for each tool.
...
- The today/roadmap boundary: what not to use (`loop`, `matrix`, `branch`) and what is shipped, such as `${{ }}` interpolation and trigger `filter` evaluation
- Two correct examples: a push-triggered checks workflow and an agent fix-until-green loop
...
The skill activates automatically when the agent detects a Shipfox context, such as a `.shipfox/` directory in the repository, or when you mention Shipfox workflows explicitly.
