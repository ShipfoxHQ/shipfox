...
provider authenticates as, a demo org, a read-only team the bot belongs to, and a
few seeded repos carrying the demo workflow and code files under `dev/gitea/seed/`.
...
The three `GITEA_*` URLs each have to be reachable from a different vantage point:
...
- **`GITEA_WEBHOOK_TARGET_URL` (`http://host.docker.internal:16101/...`)** is called
  by Gitea from inside its container, so it targets the host rather than
  `localhost`. Gitea also blocks webhooks to private/loopback hosts by default, so
  the service sets `GITEA__webhook__ALLOWED_HOST_LIST=*` (dev-only).

## Connecting the org

With `INTEGRATIONS_ENABLE_GITEA_PROVIDER=true` (already set in `apps/api/.env`),
connect the `shipfox-demo` org through the API to persist the connection and
register the org push webhook. The provider then lists the seeded repos and reads
their files; a push to Gitea delivers a webhook to the API.

## Re-seeding

`gitea-init` skips anything that already exists, so `docker compose up` is safe to
re-run. To start from a clean slate, remove the `gitea` volume:

```sh
docker compose down
docker volume rm sun-valley_gitea
```
