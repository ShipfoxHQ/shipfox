...
- **Push vs. validate by registry**: with `IMAGE_REGISTRIES` set it builds `--platform linux/amd64,linux/arm64 --push`, emitting the multi-arch manifest in one invocation (no per-arch tags, no `docker manifest` step; QEMU covers both arches). With `--image` but no registry (the PR path) it builds single-arch `--platform linux/amd64 --load` and pushes nothing.
...
- `GITHUB_SHA` / `BUILD_NUMBER` / `GITHUB_REF_NAME` — the commit identity for the `sha-<short>`, `build-<number>`, and moving `<branch>` tags. `GITHUB_SHA` also becomes the `IMAGE_REVISION` build-arg.
