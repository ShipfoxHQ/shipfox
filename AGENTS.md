...
| Adds or changes a table, migration, database query, foreign key, or transaction. | [Database boundary policy](docs/policies/database-boundaries.md) | Database ownership, access, naming, migration, and enforcement rules. Run `mise exec -- pnpm check:api-database-boundaries` and `mise exec -- pnpm check:api-migrations` locally. |
