...
The Mintlify CLI (`mint`) is a `devDependency` of `@shipfox/docs`, so `pnpm install`
provides it. It ships packages built for React 18 while this repo is React 19, so
`pnpm-workspace.yaml` allows React 19 to satisfy those peers under the strict peer
policy. The package defines no `build`/`check`/`type`/`test` scripts, so `turbo` skips
it for those. See [apps/docs/README.md](apps/docs/README.md) for details.
