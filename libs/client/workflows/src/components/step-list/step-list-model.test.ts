...
describe('buildStepListModel', () => {
  ...
  test('marks the only entry of a single-attempt step as the source anchor', () => {
    const step = makeStep({
      name: 'build',
      current_attempt: 1,
      attempts: [makeAttempt({attempt: 1})],
    });

    const result = buildStepListModel({job: makeJob({steps: [step]})});

    expect(result.entries.map((entry) => entry.isStepSourceAnchor)).toEqual([true]);
  });

  test('anchors a multi-attempt step on its current-attempt entry only', () => {
    const step = makeStep({
      name: 'build',
      current_attempt: 2,
      attempts: [
    ...
    expect(result.entries.map((entry) => `${entry.attempt}:${entry.isStepSourceAnchor}`)).toEqual([
      '1:false',
      '2:true',
    ]);
  });

  test('falls back to the last entry when no rendered attempt matches current_attempt', () => {
    const step = makeStep({
      name: 'build',
      current_attempt: 5,
      attempts: [
        makeAttempt({attempt: 1, execution_order: 1, status: 'failed'}),
        makeAttempt({attempt: 2, execution_order: 2, status: 'failed'}),
      ],
    });
  ...
  test('anchors each step independently and ignores source_location', () => {
    const located = makeStep({
      name: 'build',
      position: 0,
      source_location: {start_line: 3, end_line: 5},
      attempts: [makeAttempt({attempt: 1, execution_order: 1})],
    });
    const unlocated = makeStep({
      name: 'deploy',
      position: 1,
      source_location: null,
      attempts: [makeAttempt({attempt: 1, execution_order: 2})],
    });

    const result = buildStepListModel({job: makeJob({steps: [located, unlocated]})});

    const anchorsByLabel = result.entries
      .filter((entry) => entry.isStepSourceAnchor)
      .map((entry) => entry.step.label);
    expect(anchorsByLabel).toEqual(['build', 'deploy']);
  });
});
