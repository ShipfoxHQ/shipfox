...
- 22ef7e3: Derives generated client route identifiers from route implementations and qualifies duplicates by route path, so generated names stay stable when routes are added.
...
- Updated dependencies [3d1afa1]
  ...
  - @shipfox/client-config@46.0.1
  - @shipfox/client-ui@46.0.1
...
- b9d7c01: Adds authoritative user and workspace context to client analytics captures.
...
- 5ae8b3d: Adds `adoptSession` to enter an externally minted session that renews near expiry and falls back to the cookie session. Adds `releaseAdoptedSession` to end an adoption and restore the cookie session. Adds the optional `impersonatorId` to `AuthenticatedSession`.
