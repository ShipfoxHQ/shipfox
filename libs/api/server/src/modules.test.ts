...
describe('defaultModules', () => {
  ...
  it('injects Workflows into integrations and logs and namespaces provider secrets', async () => {
    ...
    await Promise.all([
      ...
      integrationsOptions.secrets.posthog.getSecret({...scope, key: 'token'}),
      integrationsOptions.secrets.posthog.setSecrets({
        ...scope,
        values: {token: 'secret'},
        editedBy: undefined,
      }),
      ...
      integrationsOptions.secrets.notion.deleteSecrets({...scope, keys: ['token']}),
    ...
    expect(mocks.getSecret.mock.calls.map(([params]) => params)).toContainEqual({
      key: 'token',
      namespace: 'system/integrations/notion/workspace',
      projectId: null,
      workspaceId: scope.workspaceId,
    });
    expect(mocks.setSecrets.mock.calls.map(([params]) => params)).toContainEqual({
