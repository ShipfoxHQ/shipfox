...
describe('agent provider catalog route', () => {
  ...
  describe('GET /agent/provider-catalog', () => {
    it('returns 401 without client auth', async () => {
      const res = await app.inject({
        method: 'GET',
        url: '/agent/provider-catalog',
      });

      expect(res.statusCode).toBe(401);
    });

    it('returns providers with supported models and unsupported empty model lists', async () => {
      const res = await app.inject({
        method: 'GET',
        url: '/agent/provider-catalog',
