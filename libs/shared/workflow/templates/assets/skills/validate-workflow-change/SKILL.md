...
3. For an integration trigger, call `list_trigger_events` with `source` and `event` arrays containing the workflow's source and event name, plus `replayable: true`. Read `trigger_events`; page with `cursor: next_cursor` if needed. If no matching event is retained, report that only the shape was checked and stop. Do not claim the event filter passed.
